...
if (item.name.endsWith('.pdf')) {
  ...
  size = item.size < 1048576 ? (((item.size)/(1024)).toFixed(2) + " KB"):(((item.size)/(1024*1024)).toFixed(2) + " MB")
...
url = `/file_viewer/program/index.html?target=${item.html_url}&style=atom-one-dark&type=code&showBorder=on&showLineNumbers=on&showFileMeta=on&showCopy=on&fetchFromJsDelivr=on&url=${item.url}`
...
url = `/file_viewer/program/index.html?target=${item.html_url}&style=atom-one-dark&type=ipynb&showBorder=on&showLineNumbers=on&showFileMeta=on&showCopy=on&fetchFromJsDelivr=on&url=${item.url}`
}
...
else if (item.name.endsWith('.txt') || item.name.endsWith('.url'))
{
...
} else if (item.name.endsWith('html'))
{
  ...
  icon = `<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="150" height="150" viewBox="0 0 50 50">
...
<td style="border-right-width: 2px;">${size}</td>
</tr>`
} else {
